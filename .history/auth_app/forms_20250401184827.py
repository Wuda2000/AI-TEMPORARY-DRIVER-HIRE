...
class CarOwnerRegistrationForm(forms.ModelForm):
    ...
    class Meta:
        model = CarOwner
        fields = ['name', 'phone_number', 'email']  # Personal details only
...
class CarForm(forms.ModelForm):
    class Meta:
        model = Car
        fields = ['make', 'year', 'model', 'plate_number', 'capacity', 'car_type', 'car_color', 'car_image']
...
class DriverProfileForm(forms.ModelForm):
    ...
    def clean_license_number(self):
        ...
        if self.instance and self.instance.license_number != license_number:
            # Check if the new license number is already in use
            if Driver.objects.filter(license_number=license_number).exists():
                raise forms.ValidationError("This license number is already in use. Please enter a unique license number.")
