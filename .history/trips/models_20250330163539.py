...
class Trip(models.Model):
    trip_id = models.CharField(max_length=6, unique=True, default=generate_unique_id)  
    departure_time = models.DateTimeField(default=now, null=True, blank=True)  # ✅ Prevents migration issues
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="trips")  
    def __str__(self):
        return f"Trip {self.trip_id} - {self.user.username}"
